...
| **gin-outline** | 大纲层（Stage 4） | 达标卡（一题一文）→ 大纲 `.md`（人读目录表）+ 大纲 `.json`（施工件）+ 缺口清单（外置）；方法论子知识库 `references/outline-methodology.md` + 锁死模板 `references/output-templates.md`；机检 10 条：素材存在/卡片达标/每节证据/施工说明必填/定义式标题❌/术语词收尾⚠️/三决策/节数5-8/v1格式拒认 | 19 |
...
| **gin-qc** | 质检层（Stage 6） | 一篇 Markdown 正文 → 活人感质检报告（score+verdict+L4 人审项）；四层体系教程化：段落≤300字(硬规则)/中位数句长集中度/疑问句/证据密度按行分布(引文年份和L路径不算数)/认知灰度/身份共鸣/列表背靠背；扣主线句/推测标注；L4 六项（温度感·独特性·姿态·心流·叙事弧线·对立面承认）只报告人审不进评分 | 27 |
