...
| **gin-outline** | 大纲层（Stage 4） | 达标卡（一题一文）→ 大纲 `.md`（人读目录表）+ 大纲 `.json`（施工件）+ 缺口清单（外置）；方法论子知识库 `references/outline-methodology.md` + 锁死模板 `references/output-templates.md`；机检 12 条：素材存在/卡片达标/每节证据/施工说明必填/定义式标题❌/术语词收尾⚠️/三决策/节数5-8/v1格式拒认/构件合法值锁死❌(§4.5)/篇名文档腔⚠️(§3.5) | 19 |
| **gin-draft** | 写作层（Stage 5） | 大纲.json 一篇（v2 一题一文：问题/写给/读完能/小节/素材）+ 资料库 → 一篇 Markdown 正文（篇构件，装配无关）；机检：素材锚定率≥60%（堵凭空发挥+贴名凑数）、篇首契约（写给谁+读完能做什么）、篇末交付物（清单/表/模板）、贯穿人物出场、句式密度、卡兹克禁区词表、回环呼应（篇末回扣篇首）、用人话写（空洞概括拦截） | 22 |
| **gin-qc** | 质检层（Stage 6） | 一篇 Markdown 正文 → 活人感质检报告（score+verdict+L4 人审项）；四层体系教程化：段落≤300字(硬规则)/中位数句长集中度/疑问句/证据密度按行分布(引文年份和L路径不算数)/认知灰度/身份共鸣/列表背靠背；扣主线句/推测标注；AI腔结构四查(排比≥2/段末总结>30%/'让我们'/程度副词≥3)；排版构件四查(标题跳级❌/孤立H3/方法节缺步骤/加粗>10处每千字)；L4 六项（温度感·独特性·姿态·心流·叙事弧线·对立面承认）只报告人审不进评分 | 27 |

`gin-question` 的问题清单可以直接作为 `source-scan` 的 coverage 检查输入，**无需问用户"该写什么"，机器自己发现缺口**。答案卡经 `gin-outline` 一题一文扩成单篇大纲（目录表审核件 + json 施工件），`gin-draft` 按大纲.json 整篇施工——写出的篇是装配无关的构件，未来的书从同一套篇构件装配。
